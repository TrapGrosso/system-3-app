...
const getAllProspectNotes = async (user_id, prospect_id) => {
  if (!user_id) {
    console.warn('getAllProspectNotes: user_id is not defined. Returning null.')
    return null
  }
  const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/getAllProspectNotes?user_id=${user_id}&prospect_id=${prospect_id}`, {
    method: 'GET',
    headers: {
