...
const getDeepSearchQueueItems = async (user_id) => {
  if (!user_id) {
    console.warn('getDeepSearchQueueItems: user_id is not defined. Returning null.')
    return null
  }
  const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/getDeepSearchQueueItems?user_id=${user_id}`, {
    method: 'GET',
    headers: {
