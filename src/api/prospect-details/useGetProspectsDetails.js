...
const getProspectDetails = async (user_id, prospect_id) => {
  if (!user_id) {
    console.warn('getProspectDetails: user_id is not defined. Returning null.')
    return null
  }
  const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/getProspectDetails?user_id=${user_id}&prospect_id=${prospect_id}`, {
    method: 'GET',
    headers: {
